...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand"><span className="brand-mark"><img src="./icon-192.png" alt=""/></span><div><b>练迹</b><small>FITLOG · DAILY TRAINING</small></div></div><CyberClock/><div className="streak" title={syncing ? '正在同步云端数据' : '云端数据已连接'}><Flame size={15} fill="currentColor"/> {streakOf(workouts)} 天</div></header>
    <main>
...
function Xiaohei({ pose = 'ready' }: { pose?: 'ready' | 'happy' | 'curious' | 'side' }) {
  return <span className={`xiaohei xiaohei-${pose}`} role="img" aria-label="罗小黑陪练"/>
}
...
function Today({ draft, setDraft, plans, setPlans, templates, save, share }: { draft: Workout; setDraft: (w: Workout) => void; plans: PartPlans; setPlans: (p: PartPlans) => void; templates: Exercise[]; save: () => void; share: () => void }) {
  ...
  const coach = mode === 'split'
    ? { pose: 'curious' as const, label: '小黑的动作仓库', title: '先排好动作，训练时更专注。', hint: `${plans[activePart].length} 个${activePart}部动作待命` }
    : !target
      ? { pose: 'ready' as const, label: '小黑陪你开练', title: '今天想练点什么？', hint: '选择一个或多个部位开始' }
      : done >= target
        ? { pose: 'happy' as const, label: '今日目标达成', title: '漂亮！小黑都记住了。', hint: `${done} 组全部完成` }
        : { pose: 'side' as const, label: '训练进行中', title: '稳住节奏，再来一组。', hint: `还差 ${target-done} 组完成目标` }
  ...
  const setCompletedSets = (id: string, count: number) => setDraft({...draft, exercises: draft.exercises.map(e => e.id === id ? {...e, completedSets: Math.max(0, count)} : e)})
  ...
  const card = (e: Exercise, i: number, template = false) => {
    const completed=e.completedSets||0
    const bonusCount=Math.max(2, completed-e.sets+1)
    const totalCircles=e.sets+bonusCount
    const percent=e.sets ? Math.min(100, Math.round(completed/e.sets*100)) : 0
    return <div className="exercise-card rich cyber-card" key={e.id}><span className="index">{String(i+1).padStart(2,'0')}</span><div className="exercise-main"><div className="exercise-title"><b>{e.name}</b><i>{template?'计划':e.part}</i></div><small>{e.sets} 目标组 × {e.reps} 次 {e.weight>0&&`× ${e.weight} kg`}{!!e.bonusSets&&` · 建议 +${e.bonusSets} 组`}</small>{e.cue&&<p>{e.cue}</p>}{!template&&<div className="set-orbit"><div className="set-circles">{Array.from({length:totalCircles},(_,index)=>{const number=index+1;const bonus=number>e.sets;const checked=number<=completed;return <button key={number} type="button" className={`${bonus?'bonus':''} ${checked?'checked':''}`} aria-label={`${checked?'取消':'完成'}第 ${number} 组${bonus?'（超频组）':''}`} onClick={()=>setCompletedSets(e.id,checked?number-1:number)}><span>{number}</span></button>})}</div><div className="set-summary"><span><b>{Math.min(completed,e.sets)}/{e.sets}</b> 目标组{completed>e.sets&&<em>+{completed-e.sets} 超频</em>}</span><strong>{percent}%</strong></div></div>}</div><div className="card-actions">{template&&<button aria-label="编辑" onClick={()=>openEdit(e)}><Pencil/></button>}<button aria-label="删除" onClick={()=>remove(e.id)}><Trash2/></button></div></div>
  }
  ...
  return <section className="page today-page cyber-page">
    ...
    <section className="mascot-coach"><div className="mascot-art"><Xiaohei pose={coach.pose}/><i/><i/></div><div className="mascot-copy"><small>{coach.label}</small><h1>{coach.title}</h1><p>{coach.hint}</p></div><span className="mascot-pulse"><i/></span></section>
    {mode === 'overview' ? <>
...
function HistoryView({ workouts, open }: { workouts: Workout[]; open: (w: Workout) => void }) {
  ...
  return <section className="page history-page"><div className="page-story-head"><div><div className="eyebrow">ARCHIVE / 训练档案</div><h1>你的每次<br/><em>坚持都算数。</em></h1></div><Xiaohei pose="curious"/></div><div className="month-switch"><button onClick={() => move(-1)}><ChevronLeft/></button><b>{month.getFullYear()} 年 {month.getMonth()+1} 月</b><button onClick={() => move(1)}><ChevronRight/></button></div><div className="history-summary"><div><b>{monthWorkouts.length}</b><span>训练次数</span></div><div><b>{monthWorkouts.reduce((s,w)=>s+w.duration,0)}</b><span>总分钟</span></div><div><b>{Math.round(monthWorkouts.reduce((s,w)=>s+volumeOf(w),0)/100)/10}k</b><span>总容量 kg</span></div></div><div className="timeline">{monthWorkouts.map(w => <button className="timeline-item" key={w.date} onClick={() => open(w)}><div className="timeline-date"><b>{new Date(`${w.date}T12:00:00`).getDate()}</b><span>{new Intl.DateTimeFormat('zh-CN',{weekday:'short'}).format(new Date(`${w.date}T12:00:00`))}</span></div><div className="timeline-line"><i/></div><div className="timeline-card"><div><b>{w.exercises.map(e=>e.name).slice(0,2).join(' · ')}</b><small><Clock3/> {w.duration} 分钟 · {w.exercises.length} 个动作</small></div><ChevronRight/></div></button>)}{!monthWorkouts.length && <div className="no-history"><Xiaohei pose="side"/><b>这个月还没有训练</b><span>去留下第一条记录吧</span></div>}</div></section>
}
...
function Stats({ workouts }: { workouts: Workout[] }) {
  ...
  return <section className="page stats-page"><div className="page-story-head"><div><div className="eyebrow">PROGRESS / 数据洞察</div><h1>看见你的<br/><em>持续进步。</em></h1></div><Xiaohei pose="happy"/></div><div className="big-stat"><div><span>累计训练</span><b>{total}</b><small>次</small></div><Trophy/></div><div className="dual-stats"><div><Clock3/><span>训练时长</span><b>{Math.floor(minutes/60)}<small> 小时 </small>{minutes%60}<small> 分</small></b></div><div><Dumbbell/><span>训练容量</span><b>{(volume/1000).toFixed(1)}<small> 吨</small></b></div></div><div className="chart-card"><div className="card-head"><div><small>最近 7 天</small><b>训练时长</b></div><span>{last7.reduce((s,x)=>s+x.value,0)} 分钟</span></div><div className="bar-chart">{last7.map((x,i)=><div className="bar-col" key={i}><div className="bar-track"><i style={{height:`${Math.max(5,x.value/max*100)}%`}} className={x.value ? 'filled':''}/></div><span>{x.label}</span></div>)}</div></div><div className="favorites"><div className="section-heading"><div><span>TOP</span><h2>常练动作</h2></div></div>{favorites.map(([name,count],i)=><div className="favorite-row" key={name}><span>0{i+1}</span><b>{name}</b><div><i style={{width:`${Math.max(20,count/(favorites[0]?.[1]||1)*100)}%`}}/></div><small>{count} 组</small></div>)}</div><button className="backup-btn" onClick={backup}><Download/> 导出训练数据备份 <small>JSON</small></button></section>
}
